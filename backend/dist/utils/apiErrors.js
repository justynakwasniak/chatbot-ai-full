...
exports.USER_ERRORS = {
    ...
    DAILY_LIMIT: (limit) => `Daily limit reached (${limit} messages per user). Try again tomorrow.`,
};
function logServerError(context, error) {
    console.error(`[${context}]`, error);
}
function getUserError(error, fallback) {
    ...
    if (error instanceof Error && error.message.includes('Conversation not found')) {
        return exports.USER_ERRORS.CONVERSATION_NOT_FOUND;
    }
    return fallback;
}
